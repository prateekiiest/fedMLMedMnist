...
from sampling import oct_iid, oct_noniid, oct_noniid_unequal
...
def get_dataset(args):
    ...
    if args.dataset == 'mnist':
        ...
        if(args.subset == True):
            train_client1_raw, train_client2_raw , test_client1_raw,test_client2_raw = getDataClientSubset_OCT(0.5)
        ...
        if args.iid:
            # Sample IID user data from Mnist
            user_groups = oct_iid(train_client1_dataset,train_client2_dataset)
        else:
            # Sample Non-IID user data from Mnist
            if args.unequal:
                # Chose uneuqal splits for every user TODO: to be changed later
                user_groups = oct_noniid_unequal(train_client2_dataset, args.num_users)
            else:
                # Chose euqal splits for every user TODO: to be changed later
                user_groups = oct_noniid(train_client2_dataset, args.num_users)
